...
class Board(Frame):
    ...
    def production_line_setting(self):
        ...
        self.production_line_label.grid(row=8, column=0, pady=9)
        self.brand_name_label.grid(row=9, column=0, pady=9)
        self.brand_quantity_label.grid(row=10, column=0, pady=9)
        self.production_date_label.grid(row=11, column=0, pady=9)
        self.production_button.grid(row=12, column=1, pady=9)
        self.brand_name_input.grid(row=9, column=1, pady=9,columnspan=2)
        self.brand_quantity_input.grid(row=10, column=1, pady=9,columnspan=2)
        self.production_date_input.grid(row=11, column=1, pady=9,columnspan=2)
    def insert_data_in_products_data(self, data_set=[]):
        client1 = pymongo.MongoClient("mongodb://localhost:27017/")
        db = client1.Track_and_Trace_data
            ...
            if self.server_configuration_input[i]!=None:
                ...
                if i==0 and self.server_configuration_input[i].index("end") == 0:
                    ...
                    if self.server_configuration_input[i].index("end")==0:
                        self.server_configuration_input[i].insert(INSERT, 2002)
            else:
    ...
    def server_started(self, ip_address='0.0.0.0', port_no=2002):
        ...
        if (self.inputs[0].index("end") == 0 or self.inputs[1].index("end") == 0):
            ...
            #print(data)
            if (len(data)>0):
                self.master_key=data[0]['master_key']
                for i in range(2):
                    text = ""
                    if i == 0:
                        text = "Distiller's Name:"
                        dist_data = data[0]["manufacturer_name"]
                    else:
                        text = "Established Date:"
                        dist_data = data[0]["established_date"]
                    if self.distiller_property[i] != None:
